<!-- vcp-artifact: PROJECT_MAP_REPOSITORY -->
<!-- vcp-version: v0.6.0 -->
<!-- methodology-version: v1.4 -->

# PROJECT_MAP.md

## Overview

- Vibe Coding Protocols is a controlled AI delivery framework.
- Main outcome: safer routing, adoption, hardening, maintenance, public-growth structure, integrations review, operations follow-up, backlog discipline, review gates, public-site readiness and release discipline.

## Entrypoints

- README: `README.md`
- intake: `AI_INTAKE.md`
- route chooser: `START_HERE.md`
- docs map: `docs/README.md`
- protocol index: `docs/protocol-index.md`
- CLI docs: `docs/cli.md`
- backlog docs: `docs/project-backlog.md`
- operations docs: `docs/production-observability.md`
- public-growth docs: `docs/geo-ai-visibility.md`, `docs/page-templates.md`
- local CLI: `python3 -m vcp_cli`
- local npm wrapper: `npm run vcp -- ...`

## Product layer

- CLI modules: `vcp_cli/`
- Node wrapper: `bin/vcp-node.js`
- manifests: `.vcp/manifests/`
- manifest docs: `.vcp/README.md`
- benchmarks: `benchmarks/ai-adoption/`
- demo docs: `docs/demo.md`, `docs/demo-output.md`
- route map: `docs/route-map.md`

## Main routes

- Starter
- Hardening
- Maintenance Refactoring
- UI Component Ownership
- Third-party API Intake
- Public Growth Playbook
- Operations Feedback Loop
- Project Backlog Workflow
- Post-Task Code Review
- Public Site Readiness
