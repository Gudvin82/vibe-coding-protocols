...
def evaluate_payload(root: Path | None = None) -> dict[str, object]:
    ...
    payload = {
        ...
        "track_model_present": (root / "docs/track-model.md").exists(),
        "mvp_to_launch_path_present": (root / "docs/mvp-to-launch-path.md").exists(),
        "launch_decision_checklist_present": (root / "docs/launch-decision-checklist.md").exists(),
        "mvp_to_launch_workflow_present": runtime_path_exists(root, ".vcp/workflows/mvp-to-launch.json"),
...
def run(json_mode: bool = False, print_prompt: bool = False) -> int:
    ...
    print(f"Track model present: {'yes' if payload['track_model_present'] else 'no'}")
    print(f"MVP-to-Launch path present: {'yes' if payload['mvp_to_launch_path_present'] else 'no'}")
    print(f"Launch decision checklist present: {'yes' if payload['launch_decision_checklist_present'] else 'no'}")
    print(f"MVP-to-Launch workflow present: {'yes' if payload['mvp_to_launch_workflow_present'] else 'no'}")
