...
for file in README.md README_ru.md START_HERE.md docs/lite-adoption-path.md prompts/use-this-repo-prompt.md prompts/use-this-repo-prompt_ru.md templates/README.md; do
  if [[ ! -f "$file" ]]; then
    echo "Missing onboarding file: $file"
    problems=$((problems + 1))
    continue
  fi
  if ! grep -F "root \`AGENTS.md\`" "$file" >/dev/null 2>&1 || ! grep -F "templates/AGENTS.md" "$file" >/dev/null 2>&1; then
